"""Fail-closed aggregation for the two fixed V9 paired-twist probes."""
...
from .paired_twist_set_future import AnonymousPairedTwistSetProbe
...
def _load_candidate(root: Path) -> tuple[dict, dict, dict]:
    ...
    if manifest.get("schema_version") != RUN_SCHEMA:
        raise ValueError(f"v9 probe schema differs: {root}")
    ...
    if Path(args.output).resolve() != root.resolve():
        raise ValueError(f"v9 probe output contract differs: {root}")
    count = _state_parameter_count(AnonymousPairedTwistSetProbe, args)
    checkpoint = Path(manifest["fixed_final_checkpoint"]["path"]).resolve()
    result = _validate_completed_probe(args, checkpoint, count)
    stored = json.loads((root / "probe_result.json").read_text(encoding="utf-8"))
    if _json_sha256(stored) != _json_sha256(result):
        raise ValueError(f"stored v9 result differs: {root}")
...
def _control_result(seed: int) -> dict:
    ...
    if (
        ...
        raise ValueError(f"v9 V8 control result identity differs: {seed}")
    return result


def _checks(candidate: dict, control: dict) -> dict:
    broken = candidate["broken_pairing_high_speed_velocity_mean_mps"]
    intact = candidate["high_speed_combined_velocity_mean_mps"]
    checks = {
        ...
        "high_speed_combined_velocity_improves_at_least_15_percent": (
            ...
            <= 0.85 * control["high_speed_combined_velocity_mean_mps"]
        ),
        ...
        "broken_pairing_worsens_high_speed_velocity": (
            broken > intact
            and (broken >= 1.10 * intact or broken >= intact + 0.15)
        ),
    }
    return {
...
def finalize(run_roots: list[Path], output: Path) -> dict:
    if len(run_roots) != 2 or len({path.resolve() for path in run_roots}) != 2:
        raise ValueError("v9 aggregation requires two distinct run roots")
    if output.exists():
        raise FileExistsError(f"refusing existing v9 aggregate root: {output}")
    git = _git_state()
    if git.get("worktree_dirty") is not False:
        raise ValueError("v9 aggregation requires a clean checkout")
    loaded = {}
    for root in run_roots:
        manifest, args, result = _load_candidate(root.resolve())
        seed = int(args["seed"])
        if seed in loaded:
            raise ValueError(f"duplicate v9 seed: {seed}")
        loaded[seed] = (manifest, args, result)
    if set(loaded) != set(PROBE_SEEDS):
        raise ValueError("v9 seed matrix differs")
    commits = {entry[0]["provenance"]["git"]["git_commit"] for entry in loaded.values()}
    if commits != {git.get("git_commit")}:
        raise ValueError("v9 source commit differs across runs or checkout")
    comparable_args = []
    ...
    if len({_json_sha256(value) for value in comparable_args}) != 1:
        raise ValueError("v9 arms differ outside seed-dependent fields")
    ...
    for seed in PROBE_SEEDS:
        ...
        for group_name, velocity_key, yaw_key in (
            ...
            if (
                ...
                raise ValueError(f"v9 replayed control metrics differ: {seed}/{group_name}")
        if diagnostics["overall"]["control_yaw_sign_accuracy"] != control[
            "overall_yaw_sign_accuracy"
        ]:
            raise ValueError(f"v9 replayed control sign differs: {seed}")
        args = SimpleNamespace(**{
            **loaded[seed][1], "stop_after_update": 0,
        })
        ...
        if count != v8_total_count or count != V8_JOINT_TOTAL_STATE_PARAMETERS:
            raise ValueError("v9 V8 control capacity differs by seed")
        relative_capacity = abs(
            candidate["total_state_parameter_count"]
            - V8_JOINT_REACHABLE_STATE_PARAMETERS
        ) / V8_JOINT_REACHABLE_STATE_PARAMETERS
        if relative_capacity > 0.05:
            raise ValueError("v9/V8 state capacity differs by more than 5%")
        ...
        passed_all &= passed
    report = {
        "schema_version": "stage3-v9-paired-twist-probe-aggregate-v1",
        ...
        "capacity": {
            ...
            "v9_gradient_reachable_state_parameters": loaded[PROBE_SEEDS[0]][2][
                "total_state_parameter_count"
            ],
            "v9_all_state_parameters_gradient_reachable": True,
            "matched_within_5_percent": True,
        },
        "comparisons": comparisons,
        "authorized_full_v9": passed_all,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="aggregate two fixed V9 probes")
    ...
    print(json.dumps({
        ...
        "authorized_full_v9": report["authorized_full_v9"],
    }, sort_keys=True))
