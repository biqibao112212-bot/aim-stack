...
void testWireCodecAndBounds()
{
    ...
    std::uint32_t payload_bytes = 0;
    check(
        tcp::checkedPayloadBytes(1280U, 720U, tcp::PixelFormat::Rgb24, &payload_bytes) &&
            payload_bytes == 2'764'800U,
        "maximum RGB24 payload must be exact");
    check(
        tcp::checkedPayloadBytes(1280U, 720U, tcp::PixelFormat::Rgba32, &payload_bytes) &&
            payload_bytes == 3'686'400U,
        "maximum RGBA32 payload must be exact");
    check(
        !tcp::checkedPayloadBytes(0U, 720U, tcp::PixelFormat::Rgb24, &payload_bytes) &&
            !tcp::checkedPayloadBytes(1281U, 720U, tcp::PixelFormat::Rgb24, &payload_bytes) &&
            !tcp::checkedPayloadBytes(1280U, 721U, tcp::PixelFormat::Rgba32, &payload_bytes) &&
    ...
    check(
        wire[8] == 0x00U && wire[9] == 0x01U && wire[12] == 0x00U && wire[13] == 0x00U &&
            wire[14] == 0x05U && wire[15] == 0x00U && wire[16] == 0x00U && wire[17] == 0x00U &&
            wire[18] == 0x02U && wire[19] == 0xd0U && wire[20] == 0x00U && wire[21] == 0x2aU &&
            wire[22] == 0x30U && wire[23] == 0x00U,
    ...
    const tcp::FrameHeader rgba_header = makeHeader(
        tcp::PixelFormat::Rgba32, 1280U, 720U, 7U, 8U, 9U);
    ...
    check(
        rgba_wire[8] == 0x00U && rgba_wire[9] == 0x02U &&
            rgba_wire[20] == 0x00U && rgba_wire[21] == 0x38U &&
            rgba_wire[22] == 0x40U && rgba_wire[23] == 0x00U,
