"""Train the v3 continuous translation-equivariant anonymous future model.

Mapper, S and H stay frozen and the test split stays sealed.  The model learns
one continuous future field directly; there is no future-derived expert label
or latent expert router.
"""
...
RUN_SCHEMA = "stage3-continuous-invariant-anonymous-future-oracle-pilot-v3"
MOTION_CLASSES = (2, 3)
TRAJECTORY_MODULES = (
...
def stage_trainable_parameter_ids(
    model: ContinuousInvariantAnonymousFutureModel, stage: str,
) -> set[int]:
    if stage == "trajectory":
...
def train(args: argparse.Namespace) -> Path:
    ...
    if "detach_selector_context" not in inspect.signature(
        ContinuousInvariantAnonymousFutureModel.forward
    ).parameters:
        raise RuntimeError("v3 joint isolation contract is missing")
    ...
    if bool(manifest.get("test_accessed", True)):
        raise ValueError("v3 pilot refuses a dataset that accessed test")
    if manifest.get("oracle_association") is not True:
        raise ValueError("v3 pilot requires oracle-associated data")
    ...
    source_paths = {
        "trainer_v3": Path(__file__).resolve(),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="v3 continuous invariant anonymous future diagnostic pilot"
    )
