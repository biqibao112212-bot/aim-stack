...
### 2.1 50/100/200 ms 不是距离除以弹速

本实验的 50/100/200 ms 是预先固定的评测时域。对每个锚点时刻 `t`，估计器分别外推到 `t+0.05`、`t+0.10`、`t+0.20 s`，然后与这些未来时刻的真值比较。实验没有输入弹速，没有用距离计算预测时域，也没有运行弹道解算器。

数据同时包含 `2.2/4.2/6.0 m` 三种 configured distance；每个距离都使用同一组固定时域，距离只用于分组统计。目标运动会使瞬时距离在 session 内小幅变化，但这同样不会改变本实验的预测时域。以当前 PnP 的平移 camera-CV 二维横向 P95 为例：

| configured distance | 50 ms | 100 ms | 200 ms |
| --- | ---: | ---: | ---: |
| 2.2 m | 13.8 mm | 18.7 mm | 28.8 mm |
| 4.2 m | 18.2 mm | 29.8 mm | 45.5 mm |
| 6.0 m | 35.9 mm | 40.1 mm | 48.3 mm |

因此当前总体结果混合了距离条件，且确实保留了误差随距离放大的现象；但“距离”和“未来时域”是两个独立实验变量，不能把 50/100/200 ms 解释成 2.2/4.2/6.0 m 对应的飞行时间。

真正的火控查询时刻应定义为：

```text
t_impact = t_observation + pipeline_latency + control_latency + projectile_time_of_flight
```

`projectile_time_of_flight` 的第一近似才是 `range / muzzle_speed`；考虑重力、阻力、发射姿态和目标在飞行期间继续运动后，通常需要由弹道器与目标预测器迭代求解。该 impact-time/ballistic 实验尚未执行，所以本文所有 55 mm 结论均为固定预测时域诊断，不是按实际弹速映射的命中结论。
...
本轮角点、PnP、固定时域 CV 与 11 维 EKF 证据在当前数据、oracle identity 和固定 0/50/100/200 ms 合同下视为完成并冻结，后续不再重复讨论或重跑同一问题。只有以下输入发生实质变化时才重新开启相应实验：

1. 新的角点/PnP 算法或标定使观测分布发生变化；
2. 新增非 oracle 物理身份、可见弧或 reacquisition 方法；
3. 明确弹速、系统延迟、弹道模型和 impact-time 合同，开始按距离动态选择预测时域；
