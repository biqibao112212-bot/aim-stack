...
public:
    AimSimBridgeNode() : Node("aim_sim_bridge_node")
    {
        ...
        config_.buff_config_path = declare_parameter<std::string>(
            "buff_config_path", "src/BuffDetector/buff_config.yaml");

        target_mode_param_ = declare_parameter<std::string>("target_mode", "small_buff");
        config_.default_mode = parseTargetMode(target_mode_param_, TargetMode::SmallBuff);
