...
bool readExactExposureTruthSlot(int fd, std::size_t slot_index,
                                std::uint64_t frame_seq,
                                std::uint64_t timestamp_ns,
                                ExactExposureTruth *out) {
  if (fd < 0 || out == nullptr || slot_index >= kGroundTruthHistorySlots ||
  ...
  const std::uint64_t slot_base =
      slots_base + static_cast<std::uint64_t>(slot_index) *
  ...
  if (!readFullyAt(fd, &candidate.ground_truth,
                   ...
                   slot_base +
                       offsetof(GroundTruthHistorySlot, ground_truth)) ||
      !readFullyAt(fd, &candidate.exposure_state,
                   sizeof(candidate.exposure_state),
                   slot_base +
                       offsetof(GroundTruthHistorySlot, exposure_state))) {
    return false;
  }
  std::uint64_t after = 0;
  if (!readFullyAt(fd, &after, sizeof(after), slot_base) || before != after ||
      (after & 1U) != 0 || candidate.ground_truth.frame_seq != frame_seq ||
...
bool readExactExposureTruth(int fd, const GroundTruthHistory &snapshot_hint,
                            std::uint64_t frame_seq,
                            std::uint64_t timestamp_ns,
                            ExactExposureTruth *out) {
  ...
  // The already-read metadata snapshot is only a slot index hint. The selected
  // slot is still re-read with the original before/body/after seqlock contract.
  ExactExposureTruth hinted{};
  if (selectExactExposureTruth(snapshot_hint, frame_seq, timestamp_ns,
                               &hinted) &&
      readExactExposureTruthSlot(fd, hinted.slot_index, frame_seq,
                                 timestamp_ns, out)) {
    return true;
  }

  // A producer race can invalidate the hint; preserve the legacy exact scan as
  // a fail-closed fallback rather than accepting a latest or approximate pose.
  for (std::size_t i = 0; i < kGroundTruthHistorySlots; ++i) {
    if (readExactExposureTruthSlot(fd, i, frame_seq, timestamp_ns, out))
      return true;
  }
  return false;
}
...
bool isValidMetaSnapshot(const ShmMetaRegion &meta) {
  return isCompatible(meta.header) && meta.header.created_ns != 0;
}
...
struct TcpBridgeMetrics {
  ...
  std::uint64_t metadata_read_validate_failures_after_take = 0;
...
struct BridgeStageMetric {
  ...
  void record(std::chrono::steady_clock::time_point start,
    ...
    const auto signed_ns =
        std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
            .count();
    const std::uint64_t elapsed_ns =
        signed_ns > 0 ? static_cast<std::uint64_t>(signed_ns) : 0;
    ++count;
    total_ns += elapsed_ns;
...
struct BridgeStageMetrics {
  BridgeStageMetric tcp_take_color;
  BridgeStageMetric metadata_read;
  BridgeStageMetric exact_exposure_read;
  BridgeStageMetric frame_assembly;
  BridgeStageMetric pipeline_process;
  BridgeStageMetric telemetry_write;
};

class AsyncTelemetryWriter {
public:
  AsyncTelemetryWriter(std::string path, std::string jsonl_path,
                       bool preserve_all_jsonl = false)
      : path_(std::move(path)), jsonl_path_(std::move(jsonl_path)),
        preserve_all_jsonl_(preserve_all_jsonl && !jsonl_path_.empty()),
        worker_(&AsyncTelemetryWriter::run, this) {}
  ...
  void submit(std::string payload) {
    if (path_.empty() && jsonl_path_.empty())
      return;
    {
      std::lock_guard<std::mutex> lock(mutex_);
      if (preserve_all_jsonl_) {
        queued_.push_back(std::move(payload));
      } else {
...
private:
  void run() {
    while (true) {
      std::string payload;
      {
        ...
        ready_.wait(lock, [&] {
          return stopping_ || has_pending_ || !queued_.empty();
        });
        if (!has_pending_ && queued_.empty() && stopping_)
          return;
        if (preserve_all_jsonl_) {
          payload = std::move(queued_.front());
          queued_.pop_front();
        } else {
      ...
      aim_sim_bridge::debug::writeJsonFile(path_, payload);
      aim_sim_bridge::debug::appendJsonLine(jsonl_path_, payload);
    }
  }
  ...
  bool preserve_all_jsonl_ = false;
  bool has_pending_ = false;
  bool stopping_ = false;
  std::thread worker_;
};

std::string bridgeStageMetricJson(const BridgeStageMetric &metric) {
  std::ostringstream out;
  out << std::setprecision(10) << '{';
  bool first = true;
  aim_sim_bridge::debug::appendUInt(out, "count", metric.count, first);
  aim_sim_bridge::debug::appendUInt(out, "total_ns", metric.total_ns, first);
  aim_sim_bridge::debug::appendUInt(out, "max_ns", metric.max_ns, first);
  aim_sim_bridge::debug::appendNumber(
      out, "mean_ms",
      metric.count > 0
          ? static_cast<double>(metric.total_ns) * 1e-6 /
                static_cast<double>(metric.count)
          : std::numeric_limits<double>::quiet_NaN(),
      first);
  aim_sim_bridge::debug::appendNumber(
      out, "max_ms", static_cast<double>(metric.max_ns) * 1e-6, first);
  out << '}';
  return out.str();
}
...
void writeBridgeTelemetry(
    ...
    const aim_sim_bridge::AimPipelineCounters &pipeline_stages,
    ...
    double bridge_elapsed_s, const BridgeStageMetrics &bridge_stages) {
  if (path.empty())
    return;
  ...
  aim_sim_bridge::debug::appendUInt(
      transport_json, "main_metadata_read_validate_failures_after_take",
      tcp_bridge.metadata_read_validate_failures_after_take, transport_first);
  aim_sim_bridge::debug::appendUInt(
  ...
  const auto meanStageMs = [](std::uint64_t total_ns,
  ...
  const auto pendingCount = [](std::uint64_t entered,
                               std::uint64_t completed) {
    return entered >= completed ? entered - completed : std::uint64_t{0};
  };
  std::ostringstream pipeline_stages_json;
  pipeline_stages_json << std::setprecision(10) << '{';
  bool stages_first = true;
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "timing_clock", "steady_clock_relaxed_atomic_totals",
      stages_first);
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "detector_gpu_timing_clock",
      "cuda_events_same_stream", stages_first);
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "detector_wall_partition_clock",
      "steady_clock_sampled_jobs_nonoverlapping", stages_first);
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "detector_profile_sampling",
      "launcher_job_seq_fixed_stride", stages_first);
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "detector_profile_completed_semantics",
      "sampled_completed_jobs", stages_first);
  aim_sim_bridge::debug::appendBool(
      pipeline_stages_json, "detector_profile_enabled",
      pipeline_stages.detector_profile_enabled, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_profile_timing_event_count",
      pipeline_stages.detector_profile_timing_event_count, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_profile_sample_stride",
      pipeline_stages.detector_profile_sample_stride, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_profile_completed",
      pipeline_stages.detector_profile_completed, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_profile_errors",
      pipeline_stages.detector_profile_errors, stages_first);
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "final_completion_boundary",
      "detector_pnp_tracker_aim_finalize_unique", stages_first);
  aim_sim_bridge::debug::appendString(
      pipeline_stages_json, "delivery_policy",
      "mailbox_take_only_reused_command_excluded", stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "submitted", pipeline_stages.submitted,
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "submission_rate_limited",
      pipeline_stages.submission_rate_limited, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "max_detector_submit_hz",
      pipeline_stages.max_detector_submit_hz, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_active_slots",
      pipeline_stages.detector_active_slots, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "overwritten", pipeline_stages.overwritten,
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_completed",
      pipeline_stages.detector_completed, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "solve_completed", pipeline_stages.solve_completed,
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "tracker_aim_completed",
      pipeline_stages.tracker_aim_completed, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "final_completed", pipeline_stages.final_completed,
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "delivered_completed",
      pipeline_stages.delivered_completed, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_pending_lower_bound",
      pendingCount(pipeline_stages.submitted -
                       std::min(pipeline_stages.submitted,
                                pipeline_stages.overwritten),
                   pipeline_stages.detector_completed),
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "solve_tracker_pending",
      pendingCount(pipeline_stages.detector_completed,
                   pipeline_stages.tracker_aim_completed),
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "delivery_pending_or_mailbox_overwritten",
      pendingCount(pipeline_stages.final_completed,
                   pipeline_stages.delivered_completed),
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "submission_cpu_total_ns",
      pipeline_stages.submission_cpu_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_latency_total_ns",
      pipeline_stages.detector_latency_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_raw_queue_wait_total_ns",
      pipeline_stages.detector_raw_queue_wait_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_slot_wait_total_ns",
      pipeline_stages.detector_slot_wait_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_launcher_host_total_ns",
      pipeline_stages.detector_launcher_host_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_pending_order_wait_total_ns",
      pipeline_stages.detector_pending_order_wait_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_event_wait_total_ns",
      pipeline_stages.detector_event_wait_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_fp_convert_total_ns",
      pipeline_stages.detector_fp_convert_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_postprocess_nms_total_ns",
      pipeline_stages.detector_postprocess_nms_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_completion_bookkeeping_total_ns",
      pipeline_stages.detector_completion_bookkeeping_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_profile_wall_total_ns",
      pipeline_stages.detector_profile_wall_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_gpu_h2d_total_ns",
      pipeline_stages.detector_gpu_h2d_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_gpu_preprocess_total_ns",
      pipeline_stages.detector_gpu_preprocess_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_gpu_trt_total_ns",
      pipeline_stages.detector_gpu_trt_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_gpu_d2h_total_ns",
      pipeline_stages.detector_gpu_d2h_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "detector_gpu_stream_total_ns",
      pipeline_stages.detector_gpu_stream_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "solve_cpu_total_ns", pipeline_stages.solve_cpu_ns,
      stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "tracker_aim_cpu_total_ns",
      pipeline_stages.tracker_aim_cpu_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "finalize_cpu_total_ns",
      pipeline_stages.finalize_cpu_ns, stages_first);
  aim_sim_bridge::debug::appendUInt(
      pipeline_stages_json, "pipeline_latency_total_ns",
      pipeline_stages.pipeline_latency_ns, stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "submission_cpu_mean_ms",
      meanStageMs(pipeline_stages.submission_cpu_ns, pipeline_stages.submitted),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_latency_mean_ms",
      meanStageMs(pipeline_stages.detector_latency_ns,
                  pipeline_stages.detector_completed),
      stages_first);
  const auto detectorProfileMeanMs = [&](std::uint64_t total_ns) {
    return pipeline_stages.detector_profile_completed > 0
               ? static_cast<double>(total_ns) * 1e-6 /
                     static_cast<double>(pipeline_stages.detector_profile_completed)
               : 0.0;
  };
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_raw_queue_wait_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_raw_queue_wait_ns),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_slot_wait_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_slot_wait_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_launcher_host_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_launcher_host_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_pending_order_wait_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_pending_order_wait_ns),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_event_wait_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_event_wait_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_fp_convert_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_fp_convert_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_postprocess_nms_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_postprocess_nms_ns),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_completion_bookkeeping_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_completion_bookkeeping_ns),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_profile_wall_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_profile_wall_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_gpu_h2d_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_gpu_h2d_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_gpu_preprocess_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_gpu_preprocess_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_gpu_trt_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_gpu_trt_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_gpu_d2h_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_gpu_d2h_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "detector_gpu_stream_mean_ms",
      detectorProfileMeanMs(pipeline_stages.detector_gpu_stream_ns), stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "solve_cpu_mean_ms",
      meanStageMs(pipeline_stages.solve_cpu_ns, pipeline_stages.solve_completed),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "tracker_aim_cpu_mean_ms",
      meanStageMs(pipeline_stages.tracker_aim_cpu_ns,
                  pipeline_stages.tracker_aim_completed),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "finalize_cpu_mean_ms",
      meanStageMs(pipeline_stages.finalize_cpu_ns,
                  pipeline_stages.final_completed),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "pipeline_latency_mean_ms",
      meanStageMs(pipeline_stages.pipeline_latency_ns,
                  pipeline_stages.final_completed),
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "latest_detector_latency_ms", aim.runtime_yolo_ms,
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "latest_solve_cpu_ms", aim.runtime_solve_ms,
      stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "latest_tracker_aim_cpu_ms",
      aim.runtime_track_aim_ms, stages_first);
  aim_sim_bridge::debug::appendNumber(
      pipeline_stages_json, "latest_pipeline_latency_ms",
      aim.runtime_pipeline_delay_ms, stages_first);
  pipeline_stages_json << '}';

  std::ostringstream bridge_stages_json;
  bridge_stages_json << '{';
  bool bridge_stages_first = true;
  aim_sim_bridge::debug::appendString(
      bridge_stages_json, "aggregation", "count_total_max_no_sampling",
      bridge_stages_first);
  aim_sim_bridge::debug::appendRaw(
      bridge_stages_json, "tcp_take_select_color",
      bridgeStageMetricJson(bridge_stages.tcp_take_color), bridge_stages_first);
  aim_sim_bridge::debug::appendRaw(
      bridge_stages_json, "metadata_read_validate",
      bridgeStageMetricJson(bridge_stages.metadata_read), bridge_stages_first);
  aim_sim_bridge::debug::appendRaw(
      bridge_stages_json, "exact_exposure_read",
      bridgeStageMetricJson(bridge_stages.exact_exposure_read),
      bridge_stages_first);
  aim_sim_bridge::debug::appendRaw(
      bridge_stages_json, "frame_assembly",
      bridgeStageMetricJson(bridge_stages.frame_assembly), bridge_stages_first);
  aim_sim_bridge::debug::appendRaw(
      bridge_stages_json, "pipeline_process_submit_poll",
      bridgeStageMetricJson(bridge_stages.pipeline_process),
      bridge_stages_first);
  aim_sim_bridge::debug::appendRaw(
      bridge_stages_json, "telemetry_write",
      bridgeStageMetricJson(bridge_stages.telemetry_write), bridge_stages_first);
  bridge_stages_json << '}';
  ...
  aim_sim_bridge::debug::appendRaw(out, "pipeline_stages",
                                   pipeline_stages_json.str(), first);
  aim_sim_bridge::debug::appendRaw(out, "bridge_stages",
                                   bridge_stages_json.str(), first);
...
int main(int argc, char **argv) {
  ...
  aim_sim_bridge::AimBridgeConfig config;
  config.default_mode =
      aim_sim_bridge::parseTargetMode(argValue(argc, argv, "--mode", "armor"),
                                      aim_sim_bridge::TargetMode::Armor);
  ...
  config.precision_enter_distance_m =
      argDouble(argc, argv, "--precision-enter-distance", 3.2);
  config.precision_leave_distance_m =
      argDouble(argc, argv, "--precision-leave-distance", 2.8);
  config.precision_fov_margin_px =
  ...
  const std::string fov_debug_jsonl =
      aim_sim_bridge::debug::envPath("AIM_SIM_DEBUG_FOV_JSONL");
  const int fov_target_number =
      parseIntValue(envValue("AIM_SIM_ACTIVE_TARGET_NUMBER", ""))
          .value_or(-1);
  ...
  BridgeStageMetrics bridge_stage_metrics;
  AsyncTelemetryWriter telemetry_writer(
      bridge_debug_json, bridge_debug_jsonl,
      envBoolValue("AIM_SIM_DEBUG_BRIDGE_PRESERVE_JSONL", false));
  AsyncTelemetryWriter fov_writer("", fov_debug_jsonl, true);
  ...
  const auto write_telemetry_if_due =
      [&](std::chrono::steady_clock::time_point now, bool following) {
        ...
        const auto telemetry_write_started = std::chrono::steady_clock::now();
        writeBridgeTelemetry(
            bridge_debug_json, bridge_debug_jsonl, &telemetry_writer,
            config.default_mode,
            ...
            bridge_stage_metrics);
        bridge_stage_metrics.telemetry_write.record(
            telemetry_write_started, std::chrono::steady_clock::now());
  ...
  while (true) {
    if (image_transport == ImageTransport::Tcp) {
      // TCP_METADATA_ADMISSION_TRY_TAKE: a full metadata snapshot is admitted
      // only after the latest-only mailbox yields a frame.
      if (!tcp_image_receiver ||
    ...
    // TCP_METADATA_ADMISSION_SNAPSHOT_AFTER_TAKE: file transport retains its
    // original loop-head snapshot, while TCP reaches this read only with a
    // destructively taken latest frame.
    const auto metadata_read_started = std::chrono::steady_clock::now();
    if (!readFullyAt(meta_file.fd(), &meta_snapshot, sizeof(meta_snapshot),
                     0)) {
      if (image_transport == ImageTransport::Tcp) {
        bridge_stage_metrics.metadata_read.record(
            metadata_read_started, std::chrono::steady_clock::now());
        ++image_transport_telemetry.tcp_bridge
              .metadata_read_validate_failures_after_take;
        write_telemetry_if_due(std::chrono::steady_clock::now(), was_following);
      }
    ...
    if (image_transport == ImageTransport::File) {
      ...
      const auto tcp_take_color_started = std::chrono::steady_clock::now();
      ...
      bridge_stage_metrics.tcp_take_color.record(
          tcp_take_color_started, std::chrono::steady_clock::now());
    ...
    const auto exact_exposure_read_started = std::chrono::steady_clock::now();
    ...
    const bool has_exact_exposure_truth = readExactExposureTruth(
        meta_file.fd(), meta_snapshot.ground_truth_history, image_meta.seq,
